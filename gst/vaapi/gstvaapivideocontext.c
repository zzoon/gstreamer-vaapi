    ...
    GstVaapiDisplay ** display_ptr)
{
  ...
  } else if (!g_strcmp0 (type, GST_VAAPI_DISPLAY_HANLDE_CONTEXT_TYPE_NAME)) {
    VADisplay va_display = NULL;
    structure = gst_context_get_structure (context);

    if (gst_structure_get (structure, "display", G_TYPE_POINTER, &va_display,
            NULL)) {
#if USE_X11
    ...
    GstVaapiDisplay ** display_ptr)
{
  ...
  _gst_context_query (element, GST_VAAPI_DISPLAY_HANLDE_CONTEXT_TYPE_NAME);

  if (*display_ptr)
    GST_LOG_OBJECT (element, "got a display with va display (%p)",
        *display_ptr);
